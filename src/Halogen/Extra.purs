...
mapSubmodule
  ...
  -> HalogenM state' action' slots msg m
       ~> HalogenM state action slots msg m
mapSubmodule lens wrapper halogen = (imapState lens <<< mapAction wrapper)
  halogen
...
mapComponentAction
  ...
  -> ComponentHTML action slots m
mapComponentAction actionWrapper rendered = bimap (map actionWrapper)
  actionWrapper
  rendered
...
lifeCycleEventProperty
  :: forall r action
   . { onInit :: Maybe action, onFinalize :: Maybe action }
  -> IProp r action
lifeCycleEventProperty handlers =
  (unsafeCoerce :: Prop (Input action) -> IProp r action) $ Core.ref
    onLifecycleEvent
  where
...
lifeCycleSlot
  ...
  -> ComponentHTML action
       (lifeCycleSlot :: forall query. Slot query LifecycleEvent String | slots)
       m
