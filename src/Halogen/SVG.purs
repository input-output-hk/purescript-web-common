...
svg attributes = elementNS svgNS (ElemName "svg")
  (Array.snoc attributes (xmlns svgNS))
...
svgText attributes content = elementNS svgNS (ElemName "text") attributes
  [ text content ]
...
gradientUnits
  :: forall r i. GradientUnits -> IProp (gradientUnits :: GradientUnits | r) i
...
instance isAttrBox :: IsAttr Box where
  toAttrValue (Box box) = joinWith " "
    (show <$> [ box.x, box.y, box.width, box.height ])
...
instance isAttrTranslate :: IsAttr Translate where
  toAttrValue (Translate translate) = "translate" <> parens
    (joinWith "," (show <$> [ translate.x, translate.y ]))
...
instance isAttrRGB :: IsAttr RGB where
  toAttrValue (RGB { red, green, blue }) = "rgb" <> parens
    (joinWith "," (show <$> [ red, green, blue ]))
