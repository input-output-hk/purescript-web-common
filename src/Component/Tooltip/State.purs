...
tooltip
  ...
  -> ComponentHTML action
       (tooltipSlot :: forall query. Slot query Void ReferenceId | slots)
       m
tooltip message reference placement = slot _tooltipSlot reference component
...
getElementById' id = map (bindFlipped HTMLElement.fromElement)
  $ getElementById id
  ...
  mPopperInstance <-
    runMaybeT do
      ...
      popperInstance <- liftEffect $ createPopper refElem tooltipElem
        { placement, modifiers, strategy: Absolute }
      ...
      acceptsHover <- liftEffect $ matches =<< matchMedia "(hover: hover)" =<<
        window
      if acceptsHover then do
        void $ MaybeT $ map pure $ H.subscribe $ eventListener
          (EventType "mouseenter")
          (HTMLElement.toEventTarget refElem)
          (const $ Just Show)
        void $ MaybeT $ map pure $ H.subscribe $ eventListener
          (EventType "focus")
          (HTMLElement.toEventTarget refElem)
